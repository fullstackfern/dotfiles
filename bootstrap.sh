...
for item in "$DOTFILES_DIR"/.[^.]*; do
  ...
  if [[ -f "$item" ]]; then
    ...
    target="$HOME/$rel"
    ln -sfn "$item" "$target" && success "$target → $item" || { error "Failed to symlink $rel"; }
  elif [[ -d "$item" ]]; then
...
$(find "$item" -type f -not -name ".DS_Store")
EOF
  fi
done
...
# Misc. Symlinks --------------------------------------------------------------
SYMLINKS="$DOTFILES_DIR/macos/symlinks.csv"
if [[ -f "$SYMLINKS" ]] && grep -qE '^[^#[:space:]]' "$SYMLINKS"; then
  info "Creating symlinks..."
  while IFS=, read -r name path; do
    [[ -z "$name" || "$name" == \#* || "$name" =~ ^[[:space:]]*$ ]] && continue
    ln -sfn "$path" "$HOME/$name" && success "$HOME/$name → $path" || { error "Failed to symlink $name"; }
  done < "$SYMLINKS"
fi
